...
class LookMLGenerator:
    ...
    def generate_dashboard_from_spec(self, spec: DashboardSpec) -> str:
        ...
        lines = [
            "# DRAFT SCAFFOLD: Generated by `demo-create lookml model`. Treat as a baseline draft ONLY.",
            "# MANDATORY BEFORE GATE 3 DEPLOY: Consult `looker-visualizations`, `looker-vis-advanced-config`,",
            "# `looker-vis-cartesian`, `looker-vis-tabular-kpi`, and `looker-vis-specialty-maps` to:",
            "#   1. Audit chart & series_types against Highcharts specs (use 'column'/'line'/'area', NEVER 'looker_column').",
            "#   2. Inject modern geometry tokens via advanced_vis_config (borderRadius, transparent backgroundColor, tooltips).",
            "#   3. Convert default pie charts to donuts (show_donut: true, inner_radius: 50) with curated palettes.",
        ...
        for el in spec.elements:
            ...
            if el.series_colors:
                ...
                for c_key, c_hex in el.series_colors.items():
                    lines.append(f'      "{c_key}": "{c_hex}"')
            if el.table_theme:
    ...
    def generate_default_dashboard_lkml(self, model_name: str, tables: list[LookMLTableSpec]) -> str:
        ...
        if month_timeline:
            # Dual-Axis Timeline with Independent Y-Axis Ranges, Bare Highcharts series_types & Modern Geometry Tokens
            ...
            elements.append(
                DashboardTileSpec(
                    ...
                    series_types=combo_series_types,
        ...
        elements.extend(
            [
                DashboardTileSpec(
                    ...
                    advanced_vis_config=(
                        '{"chart": {"backgroundColor": "transparent", "borderRadius": 8}, '
                        '"colors": ["#1A73E8", "#00897B", "#F9AB00", "#9334E6", "#E8710A", "#D93025"], '
                        '"tooltip": {"borderRadius": 8, "shadow": true}, '
                        '"legend": {"align": "center", "verticalAlign": "bottom"}}'
                    ),
                ...
                DashboardTileSpec(
                    ...
                    series_cell_visualizations={
                        volume_field: {"is_active": True},
                        primary_metric_field: {"is_active": True},
                    },
                    tab_name="Entity Breakdown",
                    row=2,
                    col=12,
