...
DEPRECATED_MCP_SERVERS: list[str] = [
    "data-designer",
    "bigquery",
    "knowledge-catalog",
]
...
def check_mcp_servers() -> list[MCPStatus]:
    """Check if required MCP servers are configured and deprecated MCP servers are pruned."""
    config = read_mcp_config()
    servers = config.get("mcpServers", {})

    results = []
    for s_name in DEPRECATED_MCP_SERVERS:
        if s_name in servers:
            results.append(
                MCPStatus(
                    server_name=s_name,
                    is_configured=False,
                    details=servers[s_name],
                    issues=[
                        f"Deprecated MCP server '{s_name}' present in mcp_config.json "
                        "(run 'demo-create pre-check --fix' to prune; replaced by direct CLI skill)"
                    ],
                )
            )
        else:
...
def patch_mcp_config() -> bool:
    """Prune deprecated MCP servers and inject any required servers in ~/.gemini/config/mcp_config.json."""
    config = read_mcp_config()
    if "mcpServers" not in config:
        config["mcpServers"] = {}

    for s_name in DEPRECATED_MCP_SERVERS:
        config["mcpServers"].pop(s_name, None)
