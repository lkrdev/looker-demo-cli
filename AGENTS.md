...
4. **Executive Dashboard Polish (`skills/looker-visualizations`, `skills/lookml-dashboard`)**: Dashboards must consult the `looker-visualizations` suite ([`skills/looker-visualizations`](skills/looker-visualizations/SKILL.md)) for optimal chart archetypes (KPI scorecards via `looker-vis-tabular-kpi`, dual-axis and volume timelines via `looker-vis-cartesian`, category shares and flows via `looker-vis-specialty-maps`, and validated Highcharts overrides via `looker-vis-advanced-config`). Dashboards must include single-value KPI banners, dual Y-axis timelines, `advanced_vis_config` rounded geometry (`borderRadius: 8`), and universal cross-filtering (`crossfilter_enabled: true`). All titles and labels in YAML **MUST be enclosed in double quotes** (`title: "..."`).
